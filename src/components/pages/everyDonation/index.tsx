...
import { EveryData } from "@/config/everydata";

const EveryDonationRipple = ({
  isCoreActive,
  onCardClick,
}: {
          ...
          >
            ...
            >
             ...
             transition-transform md:group-hover:scale-95 md:group-hover:bg-opacity-90"
              >
                <div className="flex justify-center items-center w-16 h-[200px] rounded-full mb-6 text-4xl font-bold text-white border-3 px-11 py-6 border-[#f3c60c] relative">
                  <span className="absolute inset-0 rounded-full h-[auto] border-[1px] border-[#f3c60c] box-border shadow-[0_0_12px_2px_rgba(243,198,12,0.5),inset_0_0_12px_2px_rgba(243,198,12,0.5)]"></span>
                  <span className="relative z-10">${value.id}</span>
                </div>
                <h2 className="text-[22px] 4xl:text-xl font-cambria mb-4 px-1">
                  {value.title}
                </h2>
                <p className="px-6 text-[14px] 2xl:text-sm font-light mb-4 leading-relaxed">
...
export default EveryDonationRipple;
