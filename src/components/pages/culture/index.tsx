...
import { CultureData } from "@/config/data";
...
const CultureSection = ({
  ...
  useEffect(() => {
    if (isActive) {
      const ctx = gsap.context(() => {
        ...
        // Animate slides with a stagger effect
  ...
  return (
    ...
    >
      ...
      >
        {"Culture of the Phoenix Collective".split("").map((char, index) => (
      ...
      >
        {CultureData.map((value, index) => (
          <SwiperSlide
            key={value.id}
            className="flex justify-center items-center px-4 py-6 2xl:px-10 2xl:py-14 cursor-pointer h-full"
          ...
          >
            ...
            >
              ...
              transition-transform md:group-hover:scale-95 md:group-hover:bg-opacity-90 h-full"
              >
                ...
                <h2 className="text-lg 2xl:text-xl font-cambria mb-4 px-4 min-h-14">
                  {value.title}
                </h2>
                <p className="px-10 text-xs 2xl:text-sm font-light mb-4 leading-relaxed">
